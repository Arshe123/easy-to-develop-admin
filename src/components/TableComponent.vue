    ...
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden" v-loading="isLoading">
      <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-4 justify-between items-center">
        <el-input
          ...
          v-if="showFilter && searchParam"
          @keyup.enter="refreshData"
        />
        <slot name="filters" :filterParams="filterParams" :refreshData="refreshData"/>
...
import { isInvalid } from '@/util/common';
...
const addObj = ref({
  _visible: false,
  _form: {...props.addFormDefaultValue},
  _ref: {},
...
const refreshData = () => {
  ...
  if(props.filter){
    for(let [k, v] of Object.entries(props.filter)){
      if(!isInvalid(v)){
        params[k] = v;
      }
    }
  ...
  for(let [k, v] of Object.entries(filterParams.value)) {
    if(v) params[k] = v;
  }

  if(props.searchParam && searchTerm.value) {
...
const addRow = () => {
  ...
  addObj.value._ref.validate(isValid => {
    ...
    const success = () => {
      ...
      btnLoading.value.add = false;
      addObj.value._form = {...props.addFormDefaultValue};
      refreshData();
    };
